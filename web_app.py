    ...
    if role == "Manager":
        ...
        if not pending.data:
            ...
            for asset in pending.data:
                label = asset["asset_id"] + " - " + asset["category"] + " - " + str(asset["count"]) + " fibers"
                with st.expander(label):
                    ...
                    msg = st.text_input("Send message to tech", key="msg_" + asset["id"])
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        if st.button("Approve", key="app_" + asset["id"]):
                            supabase.table("network_assets").update({"status": "Approved"}).eq("id", asset["id"]).execute()
                            supabase.table("notifications").insert({"user_id": asset["user_id"], "message": "Your entry " + asset["asset_id"] + " was approved!"}).execute()
                            st.rerun()
                    with col2:
                        if st.button("Reject", key="rej_" + asset["id"]):
                            supabase.table("network_assets").update({"status": "Rejected"}).eq("id", asset["id"]).execute()
                            supabase.table("notifications").insert({"user_id": asset["user_id"], "message": "Your entry " + asset["asset_id"] + " was rejected."}).execute()
                            st.rerun()
                    with col3:
                        if st.button("Send Message", key="send_" + asset["id"]):
                            if msg:
                                supabase.table("asset_comments").insert({"asset_id": asset["id"], "user_id": st.session_state["user"].id, "message": msg}).execute()
                                supabase.table("notifications").insert({"user_id": asset["user_id"], "message": "Manager commented on " + asset["asset_id"] + ": " + msg}).execute()
                                st.rerun()
    ...
    if role == "Technician":
            ...
            if st.button("Submit for Approval"):
                try:
                    ...
                    for mgr in managers.data:
                        supabase.table("notifications").insert({"user_id": mgr["user_id"], "message": "New entry " + aid + " submitted for approval."}).execute()
                    st.success("Submitted: " + aid)
                except Exception as e:
        ...
        if my_assets.data:
            for asset in my_assets.data:
                ...
                label = asset["asset_id"] + " - " + asset["category"] + " - " + str(asset["count"]) + " fibers - " + status_label
                with st.expander(label):
                    comments = supabase.table("asset_comments").select("*").eq("asset_id", asset["id"]).execute()
                    if comments.data:
                        st.markdown("**Messages:**")
